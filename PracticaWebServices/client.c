...
int main(int argc, char **argv){
	...
	int resCode;				/** Return code from server */
		...
		while (resCode != OK_NAME_REGISTERED){
			...
			//Give playerName.__size the size of the playername
			playerName.__size = strlen(playerName.msg);
			//Cal remote process to register playerName.msg
		...
		while (!endOfGame){
			...
			//Call getStatus to ask the server the status of the game
			...
			if(gameStatus.code == TURN_MOVE){
				...
				//Insert the chip in the column in playermove.
